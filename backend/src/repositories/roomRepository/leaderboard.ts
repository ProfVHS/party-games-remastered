 ...
 * @param roomCode - The unique identifier for the room.
 * @param playerID - The unique identifier for the player.
 * @param leaderboardData - Leaderboard data object of format: { score?: number }
 * @param multi - (OPTIONAL)
 * @returns A promise that resolves to void.
 */
...
export async function setPlayerInLeaderboard(
  ...
  const defaultLeaderboardData: LeaderboardDataType = { score: 0 };

  let leaderboardData: LeaderboardDataType;
  let multi: ChainableCommander | undefined;

  if (arg3 && typeof arg3 === 'object' && !Array.isArray(arg3) && 'score' in arg3) {
  ...
  if (multi) {
    multi.zadd(leaderboardKey, leaderboardData.score, playerID);
  } else {
    await client.zadd(leaderboardKey, leaderboardData.score, playerID);
  }
}
 ...
 * @param roomCode - The unique identifier for the room.
 * @returns A promise that resolves to an array of objects containing nicknames and score of all players in the leaderboard or null if leaderboard is empty.
 * @example output: [{ id: 'BJHSDFG27346', score: 100 }, { id: 'IAYGSDGTAYSFD35235', score: 50 }]
 * @example output: null
 */

export async function getAllPlayersFromLeaderboard(roomCode: string): Promise<{ id: string; score: number }[] | null> {
  const leaderboardKey = `room:${roomCode}:leaderboard`;
  ...
  for (let i = 0; i < leaderboardData.length; i += 2) {
    players.push({ id: leaderboardData[i], score: parseInt(leaderboardData[i + 1]) });
  }
 ...
 * @param roomCode - The unique identifier for the room.
 * @param playerID - The unique identifier for the player.
 * @returns A promise that resolves to the number of score of the player or null if player doesn't exist.
 * @example output: { score: 100 }
 * @example output: null
 */
